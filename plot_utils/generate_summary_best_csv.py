...
for network in networks_dict.keys():
  for dataset in networks_dict[network]:
    ...
    for saliency in all_saliencies:
      saliency_found = True
      for i_f in range(1, args.iterations + 1):
        summary_file = filename_prefix + saliency + '_caffe_iter'+ str(i_f) + '.npy'
        if not os.path.isfile(summary_file):
          saliency_found = False

      if saliency_found:
        print('{} {} {} found'.format(network, dataset, saliency))
      else:
...
if args.characterise:
  dfObj.to_csv('best_characterise.csv')
elif args.retrain:
  dfObj.to_csv('best_retrain.csv')
else:
  dfObj.to_csv('best_no_retraining.csv')
