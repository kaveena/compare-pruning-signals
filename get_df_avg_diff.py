...
for network in networks_dict.keys():
  for dataset in networks_dict[network]:
    ...
    for saliency in all_saliencies:
      summary_file = filename_prefix + saliency + '_python_iter1.npy'
        ...
        for saliency_reduction in reductions:
          for saliency_scaling in scalings:
            ...
            for i in range(1, args.iterations + 1):
              summary_file = filename_prefix + saliency + '_python_iter' + str(i) + '.npy'
              if os.path.isfile(summary_file):
...
if args.characterise:
  dfObj.to_csv('characterise_python.csv')
elif args.retrain:
  dfObj.to_csv('retrain_python.csv')
else:
  dfObj.to_csv('no_retraining_python.csv')
