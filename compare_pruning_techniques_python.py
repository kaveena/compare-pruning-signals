...
output_saliencies = dict()
input_saliencies = dict()
...
for j in range(pruning_net.total_output_channels): 
  ...
  for iter in range(forward_passes):
    ...
    if iter < backward_passes:
      net.backward()
    get_pruning_signals_python(pruning_net, output_saliencies, args.saliency_pointwise, args.saliency_input, args.saliency_norm, avg_a=False, avg_diff=True)
  ...
  if method == 'random':
    ...
    pruning_signal = saliency_python_scaling(pruning_net, output_saliencies, input_saliencies, True, False, args.saliency_input, scaling=args.scaling)
